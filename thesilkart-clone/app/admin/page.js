...
function packsToText(sizes) {
  ...
  return sizes.map((pack) => [pack.label, pack.price, pack.mrp || ''].join(' | ')).join('\n');
}

function textToPacks(value) {
  return value.split('\n').map((line) => {
    const [label, price, mrp] = line.split('|').map((part) => part.trim());
    return { label, price: Number(price), ...(mrp ? { mrp: Number(mrp) } : {}) };
