...
export default function ProductActions({ product, choices }) {
  ...
  return (
    <>
      ...
      <p className={styles.quantityNudge} key={`${packSize}-${quantity}`} role="status"><b>{quantity > 1 ? `${quantity} packs · ₹${lineTotal}` : `${packSize || 'Selected pack'} · ₹${lineTotal}`}</b>{lineSavings ? <span>You save ₹{formatSavingsAmount(lineSavings)} on this selection.</span> : starterPack && packSize !== starterPack ? <span>Prefer to try it first? Start with {starterPack} for ₹{getPackPrice(product, starterPack)}.</span> : <span>Choose the quantity you need; the total updates instantly.</span>}</p>
